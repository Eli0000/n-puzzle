...
class Taquin:
    ...
    def __init__(self, plate: Plate, algo_choice="standard A*", heuristic_choice: int = 0):
        ...
        self.plate_simple_array = [
            element for sous_liste in self.plate for element in sous_liste]
        self.n = len(plate)
        self.last_move_dir = None
        self.open_set: heapq = []
        self.closed_set = set()
        self.heuristic_weight = 1 if self.n < 4 else (self.n - 2) * ((self.n - 3 ) ** (self.n - 3)) 
        ...
        self.final_state_simple_array = [
            element for sous_liste in self.final_state for element in sous_liste]
        ...
        for dir in dir_array:
            ...
            if (dir == 'down'):
                if current_node[2] != 'up' and i_0 > 0:
                    ...
                    plate_simple_array = [
                        element for sous_liste in plate_copy for element in sous_liste]
                    if not (any(tuple_element[0] == current_node[4] for tuple_element in self.closed_set)):
                        g = self.g(current_node)
                        h = self.heuristic_funct(
                            plate_copy) * self.heuristic_weight
                        hash_current_plate = hash(tuple(plate_simple_array))
                        heapq.heappush(self.open_set, (g + h, id(plate_copy), 'down',
                                       g, hash_current_plate, plate_copy, current_node[4]))
                        possible_mouve.append("down")
                ...
                if current_node[2] != 'down' and i_0 < self.n - 1:
                    ...
                    if not (any(tuple_element[0] == current_node[4] for tuple_element in self.closed_set)):
                        ...
                        h = self.heuristic_funct(
                            plate_copy) * self.heuristic_weight
                        hash_current_plate = hash(tuple(plate_simple_array))
                        heapq.heappush(self.open_set, (g + h, id(plate_copy), 'up',
                                       g, hash_current_plate, plate_copy, current_node[4]))
                        possible_mouve.append("up")
                ...
                if current_node[2] != 'left' and j_0 > 0:
                    ...
                    if not (any(tuple_element[0] == current_node[4] for tuple_element in self.closed_set)):
                        ...
                        h = self.heuristic_funct(
                            plate_copy) * self.heuristic_weight
                        hash_current_plate = hash(tuple(plate_simple_array))
                        heapq.heappush(self.open_set, (g + h, id(plate_copy), 'right',
                                       g, hash_current_plate, plate_copy, current_node[4]))
                        possible_mouve.append("right")
                ...
                if current_node[2] != 'right' and j_0 < self.n - 1:
                    ...
                    if not (any(tuple_element[0] == current_node[4] for tuple_element in self.closed_set)):
                        ...
                        h = self.heuristic_funct(
                            plate_copy) * self.heuristic_weight
                        hash_current_plate = hash(tuple(plate_simple_array))
                        heapq.heappush(self.open_set, (g + h, id(plate_copy), 'left',
                                       g, hash_current_plate, plate_copy, current_node[4]))
                        possible_mouve.append("'left")

        return possible_mouve
    ...
    def mouve(self, mouve: MouveType, given_plate: Plate):
        match mouve["dir"]:
            case 'down':
                given_plate[mouve["y"] + 1][mouve["x"]
                                            ] = given_plate[mouve["y"]][mouve["x"]]
            case 'up':
                given_plate[mouve["y"] - 1][mouve["x"]
                                            ] = given_plate[mouve["y"]][mouve["x"]]
            case 'right':
                given_plate[mouve["y"]][mouve["x"] +
                                        1] = given_plate[mouve["y"]][mouve["x"]]
            case 'left':
                given_plate[mouve["y"]][mouve["x"] -
                                        1] = given_plate[mouve["y"]][mouve["x"]]
    ...
    def is_puzzle_solvable(self):
        ...
        distance_empty_tuile = abs(
            empty_pos_j - final_empty_pos_j) + abs(empty_pos_i - final_empty_pos_i)
